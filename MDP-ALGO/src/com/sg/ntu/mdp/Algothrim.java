...
public class Algothrim {
	...
	//***************************************//
	AlgoNode[][] explorationNodes = new AlgoNode[18][13];
...
//        System.out.println(exploredData[0][4]==1?"yes":"false");
		if(path==null){
//			 updateFootstep(currentTarget.getNodeRowIndex(),currentTarget.getNodeColIndex());
//			System.out.println("in LOOP");
			exploredData[currentNode.getNodeRowIndex()][currentNode.getNodeColIndex()]=1;
//			System.out.println("i m at "+currentNode);
			//TOdo:test this
			boolean escloop=false;
			
			for(int row=0;row<exploredData.length;row++){
				for(int col=0;col<exploredData[row].length;col++){
					if(exploredData[row][col]==0){
//						System.out.println("target"+row+","+col);
						if(row==0&&col==0){
							currentTarget = explorationNodes[17][0];
						}
							...
							currentTarget = explorationNodes[0][12];
						}
						else{
							if(col==0){
//								System.out.println("topc"+row+","+col);
								currentTarget = explorationNodes[-row+17][0];
							}else if(col==14){
//								System.out.println("bottomc"+row+","+col);
								currentTarget = explorationNodes[-row+18][12];
							}else if(row==0){
//								System.out.println("rightc"+row+","+col);
								currentTarget = explorationNodes[17][col-1];
							}else if(row==19){
//								System.out.println("leftc"+row+","+col);
								currentTarget = explorationNodes[0][col-1];
							}else{
								
								
								currentTarget = explorationNodes[Math.abs(-row+17)][col-1];
							}
						...
						if(path!=null){
//							System.out.println(row+","+col+" is target");
							escloop=true;
							break;
						}else{
//							System.out.println(row+","+col+" is unreachable");
							exploredData[row][col]=1;
							updateExploredArea(row,col);
						}
					}
				if(escloop)
					break;
